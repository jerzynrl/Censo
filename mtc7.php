      ...
      <div class="container clear_both padding_fix">
        ...
        <div class="row">
          ...
          <p>Horas diarias que duermen <b><?php echo "$row_counti";?></b> alumnos de Estadística Computacional, ciclo II - 2017.</p>
            ...
            Dónde: <br> ∑ =  
            <?php
            $sentencia = $conn->query("select sum(sueno) from encuesta;");
            ...
            <?php
            try{
            $sentencia = $conn->query("SELECT sueno FROM encuesta ORDER BY sueno ASC;");
            ...
            ?>
                ...
                <tr>
                   <th width="5%">N°</th>
                   <th width="10%">Horas que Duerme<br></th>
                </tr>
            <?php
        ...
        catch(PDOException$e)
        {
            ...
            if ($row_counti%2==0){
                ...
                $sentencia = $conn->query("SELECT sueno from encuesta ORDER BY sueno ASC limit $primero,$primero;");
                $primeros = $sentencia->fetchColumn();
                $sentencia = $conn->query("SELECT sueno from encuesta ORDER BY sueno ASC limit $segundo,$segundo;");
                ...
                //Obtener primeros datos
                $sentencia = $conn->query("SELECT sueno from encuesta ORDER BY sueno DESC limit $mediano,$mediano;");
        ...
        <!--/row-->
          ...
          <div class="block-web">
            ...
            Dónde: <br> 
            <?php
            $sentencia = $conn->query("SELECT sueno, COUNT(sueno) AS Veces FROM encuesta GROUP BY sueno ORDER BY COUNT(sueno) DESC limit 1");
            $moda1 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT sueno FROM encuesta where sueno = '$moda1'");
            $rmoda1 = $sentencia->rowCount();

            $sentencia = $conn->query("SELECT sueno, COUNT(sueno) AS Veces FROM encuesta GROUP BY sueno ORDER BY COUNT(sueno) DESC limit 1,2");
            $moda2 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT sueno FROM encuesta where sueno = '$moda2'");
            $rmoda2 = $sentencia->rowCount();

            $sentencia = $conn->query("SELECT sueno, COUNT(sueno) AS Veces FROM encuesta GROUP BY sueno ORDER BY COUNT(sueno) DESC limit 2,1");
            $moda3 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT sueno FROM encuesta where sueno = '$moda3'");
