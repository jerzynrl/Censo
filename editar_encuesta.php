...
$sentencia2=  $conn->query("SELECT * FROM encuesta where id_encuesta='$id2'");
 ...
 if(isset($_POST["boton"])==true)
  {
    ...
    $stmt = $conn->prepare('UPDATE encuesta SET edad= ?, peso= ?, estatura= ?, comidas= ?, snack= ?, alimento= ?, sueno= ?,  h_sueno= ?,  deporte= ?,  d_practica= ? WHERE id_encuesta= ?'); 

    $stmt->bindParam(1, $edad);
    $stmt->bindParam(2, $peso);
    $stmt->bindParam(3, $estatura);
    $stmt->bindParam(4, $comidas);
    $stmt->bindParam(5, $snack);
    $stmt->bindParam(6, $alimento);
    $stmt->bindParam(7, $sueno);
    $stmt->bindParam(8, $h_sueno);
    $stmt->bindParam(9, $deporte);
    $stmt->bindParam(10, $d_practica);
    $stmt->bindParam(11, $id_S);

    $stmt->execute();
    ?>
    <script language="javascript">
    alert("Encuesta Editada!");
...
?>
            ...
            <div class="block-web">
              ...
              <p>1. ¿Cúal es su edad?
              <input type="number" name="edad" required value="<?php echo $row2['edad']; ?>"></p><br>

              <p>2. ¿Cúal es su peso corporal en libras?
              <input type="number" name="peso" step="any" required value="<?php echo $row2['peso']; ?>"></p><br>

              <p>3. ¿Cúal es su estatura en metros?
              <input type="number" name="estatura" step="any" required value="<?php echo $row2['estatura']; ?>"></p><br>

              <p>4. ¿Con cuántas de las comidas reglamentarias cumple diariamente?
              <input list="comidas" name="comidas" required value="<?php echo $row2['comidas']; ?>">
              <datalist id="comidas">
                <option value="0">
                <option value="1">
                <option value="2">
              ...
              <input type="number" name="snack" required value="<?php echo $row2['snack']; ?>"></p><br>

              <p>6. Seleccione el tipo de alimentos que más consume:
              <input list="alimento" name="alimento" required value="<?php echo $row2['alimento']; ?>">
              ...
              <input type="time" name="sueno" required value="<?php echo $row2['sueno']; ?>"></p><br>

              <p>8. ¿A qué hora se duerme?
              <input type="time" name="h_sueno" required value="<?php echo $row2['h_sueno']; ?>"></p><br>

              <?php
              $value=1;
                if($value==$row2['deporte'])
                {
                  ?>
                    <p>9. ¿Practica algún deporte?<br>
                    <input type="radio" name="deporte" value="1" checked onclick="d_practica.disabled = false;d_hora.disabled = false" > Sí <br>
                    <input type="radio" name="deporte" value="0" onclick="d_practica.disabled = true;d_hora.disabled = true" > No <br></p><br>

                    <p>10. ¿Cuál deporte practica?
                    <input type="text" name="d_practica" value="<?php echo $row2['d_practica']; ?>"></p><br>

                  <?php
                }else if($value=!$row2['deporte']){
                  ?>
                    <p>9. ¿Practica algún deporte?<br>
                    <input type="radio" name="deporte" value="1" onclick="d_practica.disabled = false;d_hora.disabled = false" > Sí <br>
                    <input type="radio" name="deporte" value="0" checked onclick="d_practica.disabled = true;d_hora.disabled = true" > No <br></p><br>

                    <p>10. ¿Cuál deporte practica?
                    <input type="text" name="d_practica" disabled="disabled" value="<?php echo $row2['d_practica']; ?>"></p><br>
              ...
              <a href="datos.php" class="btn btn-danger" role="button">Cancelar</a>
