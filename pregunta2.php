        ...
        <div class="row">
          <div class="col-md-12">
            ...
            ?>
              ...
              <p>Pesos en libras de <b><?php echo "$row_counti";?></b> alumnos de Estadística Computacional, ciclo II - 2017.</p>

              <table class="table table-striped">
              <th colspan="11"><center><h6>TABLA DE FRECUENCIAS CON INTERVALOS EN GRUPOS DE 20 LIBRAS</h6></center></th>

              <tr>
              <th>Intervalo en Libras</th>
              <td>100-120</td>
              <td>121-140</td>
              <td>141-160</td>
              <td>161-180</td>
              ...
              <td>
                <?php
                $sentencia = $conn->query("select * from encuesta where peso>='100' && peso <=120;");
              ...
              <td>
                <?php
                $sentencia = $conn->query("select * from encuesta where peso>='121' && peso <=140;");
              ...
              <td>
                <?php
                $sentencia = $conn->query("select * from encuesta where peso>='141' && peso <=160;");
              ...
              <td>
                <?php
                $sentencia = $conn->query("select * from encuesta where peso>='161' && peso <=180;");
              ...
              <td>
                <?php
                $sentencia = $conn->query("select * from encuesta where peso>='181' && peso <=800;");
            ...
            <div class="block-web">
              <div class="block widget-notes">
                <div contenteditable="true" class="paper">
                  <b>Dato Mínimo: </b>
                  <?php
                    $sentencia = $conn->query("select MIN(peso) FROM encuesta;");
                    $row_countdmenor = $sentencia->fetchColumn();
                    echo ("$row_countdmenor"), ' lb(s).';
                  ?><br>
                  <b>Dato Máximo: </b>
                  <?php
                    $sentencia = $conn->query("select MAX(peso) FROM encuesta;");
                    $row_countdmayor = $sentencia->fetchColumn();
                    echo ("$row_countdmayor"), ' lb(s).';
                  ?><br>
                  <b>Amplitud: </b><?php
            ...
            <form action="grafica2.php" method="post">
