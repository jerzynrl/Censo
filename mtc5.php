    ...
    <div class="contentpanel">
      ...
      <div class="pull-left breadcrumb_admin clear_both">
        <div class="pull-left page_title theme_color">
          <h1>Pregunta 5</h1> ¿CUÁNTAS ENTRE snack / SNACKS COME AL DÍA?
        </div>
      </div>
      ...
      <div class="container clear_both padding_fix">
        ...
        <div class="row">
          <div class="col-md-12">
           ...
           <p>N° de entre comidas / snacks diarios de <b><?php echo "$row_counti";?></b> alumnos de Estadística Computacional, ciclo II - 2017.</p>
            ...
            Dónde: <br> ∑ =  
            <?php
            $sentencia = $conn->query("select sum(snack) from encuesta;");
            ...
            <?php
            try{
            $sentencia = $conn->query("SELECT snack FROM encuesta ORDER BY snack ASC;");
            ...
            ?>
                ...
                <tr>
                   <th width="5%">N°</th>
                   <th width="10%">Número de <br>(snack)</th>
                </tr>
            <?php
        ...
        catch(PDOException$e)
        {
            ...
            if ($row_counti%2==0){
                ...
                $sentencia = $conn->query("SELECT snack from encuesta ORDER BY snack ASC limit $primero,$primero;");
                $primeros = $sentencia->fetchColumn();
                $sentencia = $conn->query("SELECT snack from encuesta ORDER BY snack ASC limit $segundo,$segundo;");
                ...
                //Obtener primeros datos
                $sentencia = $conn->query("SELECT snack from encuesta ORDER BY snack DESC limit $mediano,$mediano;");
        ...
        <!--/row-->
          ...
          <div class="block-web">
            ...
            Dónde: <br> 
            <?php
            $sentencia = $conn->query("SELECT snack, COUNT(snack) AS Veces FROM encuesta GROUP BY snack ORDER BY COUNT(snack) DESC limit 1");
            $moda1 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT snack FROM encuesta where snack = '$moda1'");
            $rmoda1 = $sentencia->rowCount();

            $sentencia = $conn->query("SELECT snack, COUNT(snack) AS Veces FROM encuesta GROUP BY snack ORDER BY COUNT(snack) DESC limit 1,2");
            $moda2 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT snack FROM encuesta where snack = '$moda2'");
            $rmoda2 = $sentencia->rowCount();

            $sentencia = $conn->query("SELECT snack, COUNT(snack) AS Veces FROM encuesta GROUP BY snack ORDER BY COUNT(snack) DESC limit 2,1");
            $moda3 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT snack FROM encuesta where snack = '$moda3'");
            $rmoda3 = $sentencia->rowCount();

            if($rmoda3 == $rmoda2 and $rmoda3 == $rmoda1){
              ?><strong>MULTIMODAL = <?php echo $moda1;?> snack, el cual se repite <?php echo $rmoda1; ?> veces.</strong><br>
            <strong>MULTIMODAL = <?php echo $moda2;?> snack, el cual se repite <?php echo $rmoda2; ?> veces.</strong><br>
            <strong>MULTIMODAL = <?php echo $moda3;?> snack, el cual se repite <?php echo $rmoda3; ?> veces.</strong><?php
            }
            elseif($rmoda2 == $rmoda1){
              ?><strong>BIMODAL = <?php echo $moda1;?> snack, el cual se repite <?php echo $rmoda1; ?> veces.</strong><br>
            <strong>BIMODAL = <?php echo $moda2;?> snack, el cual se repite <?php echo $rmoda2; ?> veces.</strong><br><?php
            }
            else{
              ?><strong>MODA = <?php echo $moda1;?> snack, el cual se repite <?php echo $rmoda1; ?> veces.</strong><?php
            }
            ?>
            </div>
