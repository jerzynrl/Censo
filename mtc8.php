...
<title>Censo Alimenticio | Pregunta 8</title>
    ...
    <div class="contentpanel">
      ...
      <div class="pull-left breadcrumb_admin clear_both">
        <div class="pull-left page_title theme_color">
          <h1>Pregunta 8</h1> ¿A QUE HORA SE DUERME?
        </div>
      </div>
      ...
      <div class="container clear_both padding_fix">
        ...
        <div class="row">
          ...
          <p>Hora en que se duermen <b><?php echo "$row_counti";?></b> alumnos de Estadística Computacional, ciclo II - 2017.</p>
            ...
            Dónde: <br> ∑ =  
            <?php
            $sentencia = $conn->query("select sum(h_sueno) from encuesta;");
            $row_count1 = $sentencia->fetchColumn();
            echo round("$row_count1"/10000);?><br>
            n = <?php echo "$row_counti";?><br>
            <?php 
            $mediana = $row_count1/$row_counti;
            $mediana2 = $mediana/10000
            ?>
            <strong>X ̅  = <?php echo round("$mediana2",2);?></strong>
            </center>
            </div>
            ...
            <?php
            try{
            $sentencia = $conn->query("SELECT h_sueno FROM encuesta ORDER BY h_sueno ASC;");
        ...
        catch(PDOException$e)
        {
            ...
            if ($row_counti%2==0){
                ...
                $sentencia = $conn->query("SELECT h_sueno from encuesta ORDER BY h_sueno ASC limit $primero,$primero;");
                $primeros = $sentencia->fetchColumn();
                $sentencia = $conn->query("SELECT h_sueno from encuesta ORDER BY h_sueno ASC limit $segundo,$segundo;");
                ...
                //Obtener primeros datos
                $sentencia = $conn->query("SELECT h_sueno from encuesta ORDER BY h_sueno DESC limit $mediano,$mediano;");
        ...
        <!--/row-->
          ...
          <div class="block-web">
            ...
            Dónde: <br> 
            <?php
            $sentencia = $conn->query("SELECT h_sueno, COUNT(h_sueno) AS Veces FROM encuesta GROUP BY h_sueno ORDER BY COUNT(h_sueno) DESC limit 1");
            $moda1 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT h_sueno FROM encuesta where h_sueno = '$moda1'");
            $rmoda1 = $sentencia->rowCount();

            $sentencia = $conn->query("SELECT h_sueno, COUNT(h_sueno) AS Veces FROM encuesta GROUP BY h_sueno ORDER BY COUNT(h_sueno) DESC limit 1,2");
            $moda2 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT h_sueno FROM encuesta where h_sueno = '$moda2'");
            $rmoda2 = $sentencia->rowCount();

            $sentencia = $conn->query("SELECT h_sueno, COUNT(h_sueno) AS Veces FROM encuesta GROUP BY h_sueno ORDER BY COUNT(h_sueno) DESC limit 2,1");
            $moda3 = $sentencia->fetchColumn();
            $sentencia = $conn->query("SELECT h_sueno FROM encuesta where h_sueno = '$moda3'");
